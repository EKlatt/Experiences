...
void CLKchange() {
  ...
  if      ( CLK==LOW  && state==IDLE_11 ) state=CW_01;              // CLKfall
  else if ( CLK==LOW  && state==CCW_10 )  state=CCW_00;             // CLKfall
  else if ( CLK==HIGH && state==CW_00 )   state=CW_10;              // CLKrise
  else if ( CLK==HIGH && state==CCW_01 )  {state=IDLE_11; count--;} // CLKrise
  msg=true;
}
...
void DTchange() {
  ...
  if      ( DT==LOW  && state==IDLE_11 ) state=CCW_10;             //DTfall
  else if ( DT==LOW  && state==CW_01 )   state=CW_00;              //DTfall
  else if ( DT==HIGH && state==CCW_00 )  state=CCW_01;             //DTrise
  else if ( DT==HIGH && state==CW_10 )   {state=IDLE_11; count++;} //DTrise
  msg=true;
}
...
void loop() {
  // Any change in er value is displayed in Serial Monitor
  if (msg && debug) {
